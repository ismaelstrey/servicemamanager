...
export class ProviderRepository {
   ...
   */
  async list(query: ListProvidersQuery & { userProviders?: number[] }): Promise<{
    providers: Provider[];
    ...
    try {
      ...
      const mappedProviders = providers.map((provider: any) => this.mapProviderFromPrisma(provider));
   ...
   */
  async updateStatus(id: number, status: Status, updatedBy: number): Promise<Provider | null> {
    try {
   ...
   */
  async createInvite(invite: ProviderInvite): Promise<ProviderInvite> {
    try {
   ...
   */
  async findUserByEmail(providerId: number, email: string): Promise<ProviderUser | null> {
    try {
      ...
      return {
        ...
        role: userProvider.role as any,
        permissions: userProvider.permissions || [],
        status: userProvider.isActive ? 'active' : 'inactive' as any,
   ...
   */
  async findPendingInvite(token: string): Promise<ProviderInvite | null> {
    try {
      // TODO: Implementar tabela ProviderInvite no schema Prisma
  ...
  async getUsers(providerId: number): Promise<ProviderUser[]> {
    try {
      ...
      return userProviders.map((up: any) => ({
        id: up.id,
        userId: up.userId,
        providerId: up.providerId,
        role: up.role as any,
        permissions: up.permissions || [],
        status: up.isActive ? 'active' : 'inactive' as any,
        user: up.user
      }));
    } catch (error) {
   ...
   */
  async updateSettings(providerId: number, settings: Partial<ProviderSettings>, updatedBy: number): Promise<Provider | null> {
    try {
  ...
  private mapProviderFromPrisma(prismaProvider: any): Provider {
    return {
      ...
      plan: 'basic' as any,
      status: 'active' as any, // Campo padrão já que não existe no schema
      settings: {
        timezone: 'America/Sao_Paulo',
        language: 'pt-BR',
        dateFormat: 'DD/MM/YYYY',
        timeFormat: '24h' as any,
        currency: 'BRL',
        ticketSettings: {} as any,
