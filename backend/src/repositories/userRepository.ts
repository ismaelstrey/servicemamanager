...
export class UserRepository {
  // Cria usuário
  async create(name: string, email: string, passwordHash: string): Promise<{ id: number; name: string; email: string }> {
    const user = await prisma.user.create({ data: { name, email, password: passwordHash } });
    return { id: user.id, name: user.name, email: user.email };
  }

  // Busca por email
  async findByEmail(email: string): Promise<{ id: number; name: string; email: string; password: string } | null> {
