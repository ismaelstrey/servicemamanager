...
export class ProviderService {
   ...
   */
  async create(data: CreateProviderDto, createdBy: number): Promise<CreateProviderResponse> {
    try {
      ...
      const defaultSettings: ProviderSettings = {
        ...
        ticketSettings: {
          ...
          escalationRules: [] as ProviderEscalationRule[],
          slaSettings: {
            enabled: false,
            responseTime: {},
            resolutionTime: {},
   ...
   */
  async list(query: ListProvidersQuery, user: AuthUser): Promise<{
    providers: Provider[];
   ...
   */
  async findById(id: number, user: AuthUser): Promise<Provider | null> {
    try {
   ...
   */
  async findByWorkspace(workspace: string): Promise<Provider | null> {
    try {
   ...
   * Atualizar provedor
   */
  async update(id: number, data: UpdateProviderDto, updatedBy: number): Promise<UpdateProviderResponse> {
    try {
   ...
   */
  async updateStatus(id: number, status: Status, updatedBy: number): Promise<Provider> {
    try {
   ...
   */
  async getStats(providerId: number): Promise<ProviderStats> {
    try {
   ...
   */
  async inviteUser(providerId: number, data: InviteToProviderDto, invitedBy: number): Promise<ProviderInvite> {
    try {
   ...
   */
  async getUsers(providerId: number): Promise<ProviderUser[]> {
    try {
   ...
   */
  async updateSettings(providerId: number, settings: Partial<ProviderSettings>, updatedBy: number): Promise<Provider> {
    try {
   ...
   */
  async isWorkspaceAvailable(workspace: string): Promise<boolean> {
    try {
   ...
   */
  async hasPermission(user: AuthUser, providerId: number, permission: string): Promise<boolean> {
    try {
      // Papéis globais têm acesso amplo
      if (user.role === 'super_admin' || user.role === 'admin') {
        return true;
   ...
   */
  private validateCreateData(data: CreateProviderDto): void {
   ...
   */
  private async generateUniqueWorkspace(name: string): Promise<string> {
   ...
   */
  private applyUserFilters(query: ListProvidersQuery, user: AuthUser): ListProvidersQuery {
   ...
   * Verificar se usuário tem acesso ao provedor
   */
  private userHasAccessToProvider(user: AuthUser, provider: Provider): boolean {
