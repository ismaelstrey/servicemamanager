...
class AIService {
     ...
     */
    async analyzeTicketPriority(title, description, category, providerId) {
        try {
     ...
     */
    async findSimilarTickets(title, description, category, providerId) {
     ...
     */
    extractCriticalKeywords(title, description) {
    ...
    calculatePriorityScore(factors) {
        ...
        if (factors.historicalTickets.length > 0) {
            const highPriorityCount = factors.historicalTickets.filter(t => t.priority === 'high' || t.priority === 'critical').length;
     ...
     */
    async calculateHistoricalPatterns(tickets) {
        if (tickets.length === 0) {
            return {
                similarTickets: 0,
                averageResolutionTime: 0,
    ...
    async analyzeEquipmentRisk(equipment) {
        ...
        const baseRisk = equipmentRiskFactors[equipment.type] || 0.2;
     ...
     */
    generateMaintenanceRecommendations(equipment, probability, recentTicketCount) {
