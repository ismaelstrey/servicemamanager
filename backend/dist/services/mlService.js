...
class MLService {
     ...
     */
    async trainClassificationModel(providerId) {
        try {
     ...
     */
    async classifyTicket(title, description, source, providerId) {
        try {
     ...
     */
    async analyzeHistoricalPatterns(providerId) {
        try {
     ...
     */
    extractTicketFeatures(title, description, source) {
        return {
    ...
    calculatePriorityScores(features, model) {
        ...
        features.descriptionPatterns.forEach((pattern) => {
        ...
        historicalData.forEach(ticket => {
            ...
            if (!patterns[key]) {
                patterns[key] = {
                    pattern: key,
                    frequency: 0,
                    averagePriority: ticket.priority,
     ...
     */
    makePredictions(historicalData) {
     ...
     */
    generateMLRecommendations(patterns, historicalData) {
    ...
    extractDescriptionPatterns(description) {
        const patterns = [];
        const text = description.toLowerCase();
        // Padrões comuns
        if (text.includes('não funciona'))
            patterns.push('not_working');
        if (text.includes('lento'))
            patterns.push('slow_performance');
        if (text.includes('erro'))
            patterns.push('error_message');
        if (text.includes('conexão'))
    ...
    getHighestScorePriority(scores) {
        return Object.entries(scores).reduce((a, b) => scores[a[0]] > scores[b[0]] ? a : b)[0];
    }
    estimateResolutionTime(features, model) {
    ...
    calculateModelAccuracy(historicalData, features) {
