/* Smoke test for Password Vault routes */
...
async function main() {
  ...
  console.log('Listing passwords (initial)...');
  const list1 = await get(`/api/providers/${providerId}/passwords?limit=1&page=1`, token);
  console.log('List1:', list1.status, list1.data);

  console.log('Creating password entry...');
  const create = await post(`/api/providers/${providerId}/passwords`, { label: 'Demo Vault', username: 'demo', password: 's3cr3t' }, token);
  ...
  const upd = await put(`/api/providers/passwords/${passId}`, { label: 'Demo Vault Updated', password: 'n3wp@ss' }, token);
  console.log('Update:', upd.status, upd.data);

  console.log('Delete entry...');
  const delRes = await del(`/api/providers/passwords/${passId}`, token);
  console.log('Delete:', delRes.status, delRes.data);

  console.log('Listing passwords (after delete)...');
  const list2 = await get(`/api/providers/${providerId}/passwords?limit=1&page=1`, token);
